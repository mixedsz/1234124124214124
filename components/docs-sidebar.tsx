...
function SidebarSection({ section, activePath, onNavigate }: { section: NavSection; activePath: string; onNavigate?: () => void }) {
        ...
        <div className="border-t border-neutral-800 px-4 py-4 space-y-4 bg-neutral-900">
          {NAV.map(section => (
            <div key={section.title}>
              <p className="flex items-center gap-2 text-xs text-neutral-600 uppercase tracking-widest font-semibold px-2 mb-1.5">
                {section.icon}
                {section.title}
              </p>
