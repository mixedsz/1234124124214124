...
export function StoreContent({ initialCategories }: { initialCategories: TebexCategory[] }) {
  const [categories, setCategories] = useState<TebexCategory[]>(initialCategories);
  const [activeCategory, setActiveCategory] = useState<number | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(initialCategories.length === 0 ? 'Failed to load products. Please try again later.' : null);
  ...
  // If server gave no data, try client-side fetch as fallback
  useEffect(() => {
    if (initialCategories.length > 0) return;
    setLoading(true);
    fetch('/api/categories', { cache: 'no-store' })
      .then(r => r.json())
      .then((cats: TebexCategory[]) => {
